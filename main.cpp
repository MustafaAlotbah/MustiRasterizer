...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE _null, LPSTR lpszArgument, int iCmdShow) {
	...
	while (isRunning)
	{
		...
		/*------------- SIMULATE-------------*/
		for (int y = 0; y < mge::videoBuffer.height; y++)
		{
			for (int x = 0; x < mge::videoBuffer.width; x++)
			{
				*((unsigned int*)mge::videoBuffer.addr + y * mge::videoBuffer.width + x) = x;
			}
		}
		...
		StretchDIBits(
			hdc,
			0, 0,
			mge::videoBuffer.width, mge::videoBuffer.height,
			0, 0,
			mge::videoBuffer.width, mge::videoBuffer.height,
			mge::videoBuffer.addr, &(mge::videoBuffer.bitmapInfo),
...
LRESULT CALLBACK eventHandler(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_SIZE:
	{
		...
		mge::videoBuffer.width = rect.right - rect.left;
		mge::videoBuffer.height = rect.bottom - rect.left;

		int buffer_size = mge::videoBuffer.width * mge::videoBuffer.height * sizeof(unsigned int);

		// allocate memory for the buffer
		if (mge::videoBuffer.addr) VirtualFree(mge::videoBuffer.addr, 0, MEM_RELEASE);
		mge::videoBuffer.addr = VirtualAlloc(0, buffer_size, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);

		// setup new bitmap info header
		mge::videoBuffer.bitmapInfo.bmiHeader.biSize = sizeof(mge::videoBuffer.bitmapInfo.bmiHeader);
		mge::videoBuffer.bitmapInfo.bmiHeader.biWidth = mge::videoBuffer.width;
		mge::videoBuffer.bitmapInfo.bmiHeader.biHeight = mge::videoBuffer.height;
		mge::videoBuffer.bitmapInfo.bmiHeader.biPlanes = 1;
		mge::videoBuffer.bitmapInfo.bmiHeader.biBitCount = 32;
		mge::videoBuffer.bitmapInfo.bmiHeader.biCompression = BI_RGB;
	}
	break;
	// otherwise
	default:
